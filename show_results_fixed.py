"""Display scrape results comparison with proper encoding."""

import json
from pathlib import Path

print("="*70)
print("OPTIMIZED SCRAPE RESULTS WITH AI EXTRACTION & PLAYWRIGHT FIX")
print("="*70)
...
# Compare with backup
try:
    with open("jobs_db.json.backup", encoding="utf-8") as f:
        backup = json.load(f)
    ...
    if diff > 0:
        print(f"  Change: +{diff} jobs ({pct}% increase) [MAJOR IMPROVEMENT]")
    elif diff < 0:
...
output_file = Path(
    "C:/Users/TIM~1.BOS/AppData/Local/Temp/claude/C--timprojects-jobscraper/8f99508a-792a-445d-a1ad-de4cfa1a2701/tasks/bnr6bagl4.output"
)
if output_file.exists():
    with open(output_file, encoding="utf-8", errors="ignore") as f:
        ...
        playwright_count = len([l for l in lines if "SPA detected" in l])
        playwright_success = len([l for l in lines if "[HTML + AI]" in l and "returned" in l]) - html_count
        print(f"  ATS API used: {ats_count} times")
        print(f"  HTML + AI used: {html_count} times")
        print(f"  Playwright rendering triggered: {playwright_count} times")
...
print(f"\nExport: output/jobs_2026-06-21.xlsx")
print(f"\n✓ Optimization successful! Improved AI extraction and SPA handling.")
